...
public class JobServiceClient implements BackgroundResource {
   ...
   *
   * <p>Note: Jobs in OPEN status remain searchable until the operation completes.
   *
   * <p>Note: The operation returned may take hours or longer to complete, depending on the number
   * of jobs that need to be deleted.
   *
   * <p>Sample code:
   *
   * <pre>{@code
   ...
   *   PurgeJobsResponse response = jobServiceClient.purgeJobsAsync(parent, filter).get();
   * }
   * }</pre>
   *
   * @param parent Required. The resource name of the project under which the jobs should be
   *     deleted.
   *     <p>The format is "projects/{project_id}". For example, "projects/foo".
   * @param filter Required. A filter matching the jobs to be purged.
   *     <p>The filter can be one of the following three parent resources. 1. Company. Resource name
   *     of the company under which all the jobs should be deleted. The format is
   *     "projects/{project_id}/tenants/{tenant_id}/companies/{company_id}". For example,
   *     "projects/foo/tenants/bar/companies/baz" 2. Tenant. Resource name of the tenant under which
   *     all the jobs should be deleted. The format is "projects/{project_id}/tenants/{tenant_id}".
   *     For example, "projects/foo/tenants/bar". 3. Project. Resource name of the project under
   *     which all the jobs should be deleted. The format is "projects/{project_id}". For example,
   *     "projects/foo/".
   * @throws com.google.api.gax.rpc.ApiException if the remote call fails
   */
  public final OperationFuture<PurgeJobsResponse, BatchOperationMetadata> purgeJobsAsync(
  ...
  public final OperationFuture<PurgeJobsResponse, BatchOperationMetadata> purgeJobsAsync(
    ...
    PurgeJobsRequest request =
        PurgeJobsRequest.newBuilder().setParent(parent).setFilter(filter).build();
    return purgeJobsAsync(request);
  }
  ...
  public final OperationFuture<PurgeJobsResponse, BatchOperationMetadata> purgeJobsAsync(
      PurgeJobsRequest request) {
    return purgeJobsOperationCallable().futureCall(request);
  }
   ...
   * }</pre>
   */
  public final OperationCallable<PurgeJobsRequest, PurgeJobsResponse, BatchOperationMetadata>
      purgeJobsOperationCallable() {
    return stub.purgeJobsOperationCallable();
  }
   ...
   * }</pre>
   */
  public final UnaryCallable<PurgeJobsRequest, Operation> purgeJobsCallable() {
    return stub.purgeJobsCallable();
  }
   ...
   *           .setMendelDebugInput(Any.newBuilder().build())
   *           .build();
